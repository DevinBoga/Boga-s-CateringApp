...
function offerHtml(o){
 ...
 return `<div class="offerPreview oldOffer">
  ...
  <div class="letterTop">
    ...
    <div class="metaBox">
      <table>
        <tr><td>Angebot</td><td><b>A ${esc(o.number)}</b></td></tr>
        <tr><td>Datum</td><td>${heute}</td></tr>
        <tr><td>Catering-Ort</td><td>${cateringAddress||'-'}</td></tr>
        <tr><td>Asp.</td><td>Nicole Boga</td></tr>
      </table>
    </div>
  </div>
...
function buildOfferPdf(o){
  ...
  function text(x, yy, str, size=10, font='F1'){
    op(`BT /${font} ${size} Tf ${x.toFixed(2)} ${yy.toFixed(2)} Td ${pdfLiteral(str)} Tj ET`);
  }
  ...
  function image(name,x,yy,w,h){ op(`q ${w.toFixed(2)} 0 0 ${h.toFixed(2)} ${x.toFixed(2)} ${yy.toFixed(2)} cm /${name} Do Q`); }
  function drawHeader(repeat=false){
    if(window.BOGAS_LOGO){ image('Logo',235,H-105,125,125); }
    else { text(235,H-54,"Boga's",34,'F2'); text(247,H-80,'- MEETBAR -',18,'F1'); }
    line(50,H-112,545,H-112);
    y=H-140;
    if(repeat){
  ...
  function row(pos, descLines, qtyLines, epLines, totalLines){
    ...
    text(55,y,pos,12,'F2'); text(95,y,desc[0]||'',10,'F2');
    ...
    grey(0.82); line(50,y+6,545,y+6); black();
    y-=h;
  }
  function foodLines(){
    const lines=[];
  ...
  const metaY=H-150;
  text(355,metaY,'Angebot:',12); text(465,metaY,'A '+o.number,12,'F2');
  ...
  text(355,metaY-86,'Asp.:',12); text(465,metaY-86,'Nicole Boga',12);
  y=H-330;
  ...
  function logoHex(){
    try{
      if(!window.BOGAS_LOGO) return '';
      const b64=String(window.BOGAS_LOGO).split(',')[1]||'';
      const bin=atob(b64);
      let hex='';
  ...
  const logoData=logoHex();
  const logoId=logoData ? add(`<< /Type /XObject /Subtype /Image /Width 500 /Height 500 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter [/ASCIIHexDecode /DCTDecode] /Length ${logoData.length} >>\nstream\n${logoData}\nendstream`) : null;
  const pageIds=[];
  pages.forEach(content=>{
    const stream=`<< /Length ${pdfBytes(content).length} >>\nstream\n${content}\nendstream`;
    const contentId=add(stream);
    const xObj = logoId ? `/XObject << /Logo ${logoId} 0 R >>` : '';
    const pageId=add(`<< /Type /Page /Parent 0 0 R /MediaBox [0 0 ${W} ${H}] /Resources << /Font << /F1 ${font1} 0 R /F2 ${font2} 0 R >> ${xObj} >> /Contents ${contentId} 0 R >>`);
    pageIds.push(pageId);
  });
  ...
  let pdf='%PDF-1.4\n%\xE2\xE3\xCF\xD3\n', offsets=[0];
  objects.forEach((obj,i)=>{ offsets[i+1]=pdfBytes(pdf).length; pdf+=`${i+1} 0 obj\n${obj}\nendobj\n`; });
